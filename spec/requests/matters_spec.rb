...
RSpec.describe 'matters_requests', type: :request do
  ...
  describe 'POST #create' do
    before do
      ...
      # createするmatter_joinするcurrent_userまたはcurrent_office
    ...
    context '正常系' do
        ...
        context '案件、相手方等、相手方連絡先を入力' do
          before do
            @matter_params = attributes_for(:matter, matter_category_id: matter_category.id
              ).merge(@matter_join_params, @contact_opponent_params)
          end
          it 'リクエストが成功すること' do
      ...
      context 'クライアント管理個人ユーザーでログイン' do
        ...
        context '案件、相手方等、相手方連絡先を入力' do
          before{@matter_params = attributes_for(:matter, matter_category_id: matter_category.id
            ).merge(@matter_join_params, @contact_opponent_params)}
          it 'リクエストが成功すること' do
            post api_v1_client_matters_path(client), params: { matter: @matter_params }
            expect(response).to have_http_status 200
        ...
        context '案件と案件タグを入力' do
          before{@matter_params = attributes_for(:matter, matter_category_id: matter_category.id, tag_name: '案件タグ'
          ).merge(@matter_join_params)}
          it '登録されること' do
            expect do
              post api_v1_client_matters_path(client), params: { matter: @matter_params }
  ...
  describe 'PUT #update' do
    context '正常系' do
      ...
      context '案件タグを追加' do
        before{
          login_user(matter_admin_user, 'Test-1234', api_v1_login_path)
          @matter_params = attributes_for(:matter, matter_category_id: pension_matter_category.id, tag_name: 'タグ追加')
        }
        it 'リクエストが成功すること' do
          put api_v1_matter_path(matter), params: { matter: @matter_params }
          expect(response.status).to eq 200
    ...
    context '準正常系' do
      context '未ログイン' do
        ...
        it '更新されないこと' do
          expect do
            put api_v1_matter_path(matter), params: { matter: @matter_params }
          end.to_not change { Matter.find(matter.id).matter_category_id }
        end
      end
      context 'パラメータが無効' do
        before{
          login_user(matter_admin_user, 'Test-1234', api_v1_login_path)
          @matter_params = attributes_for(:matter, matter_category_id: nil)
        }
