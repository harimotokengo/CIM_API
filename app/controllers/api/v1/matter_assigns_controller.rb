class MatterAssignsController < ApplicationController
  ...
  def create
    ...
    return response_bad_request unless @matter.assignable_check(params[:user_id])
    matter_assign = @matter.matter_assigns.new(params[:user_id])
  ...
  def correct_user
    if @matter.client.active.join_check(current_user) || @matter.join_check(current_user)
      return true
    end
  end
end